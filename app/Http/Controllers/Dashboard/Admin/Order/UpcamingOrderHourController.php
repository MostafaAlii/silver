...
class UpcamingOrderHourController extends Controller {
    ...
    public function updateDate(Request $request, $orderId) {
        try {
            $this->upcamingOrderHourService->updateDate($orderId, $request->data);
    ...
    public function updateTime(Request $request, $orderId) {
        try {
            $this->upcamingOrderHourService->updateTime($orderId, $request->hours_from);
            return redirect()->route('upcamingOrderHour.index')->with('success', 'Upcaming Order Hour updated Start Time successfully');
        } catch (\Exception $e) {
            return redirect()->route('upcamingOrderHour.index')->with('error', 'An error occurred while updating the Upcaming Order Hour Start Time');
        }
    }
}